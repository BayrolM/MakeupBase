      ...
      <DialogContent className="bg-white border-0 max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl p-0">
        ...
        <div className="flex justify-end gap-3 px-6 pb-6 pt-5 bg-white border-t border-gray-100 sticky bottom-0 z-10 rounded-b-2xl">
          <Button
            ...
            disabled={isSaving}
            className="bg-white border-gray-200 text-gray-600 hover:bg-gray-50 rounded-xl px-6 h-11 text-sm font-semibold disabled:opacity-40"
          >
            Cancelar
          </Button>
          <button
            onClick={onSave}
            disabled={isSaving}
            className="rounded-xl font-bold px-8 h-11 text-sm border-0 luxury-button-modal shadow-lg shadow-[#c47b96]/20 flex items-center justify-center text-white disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isSaving ? (
              <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Guardando...</>
            ) : (
              <><CheckCircle2 className="w-4 h-4 mr-2" /> {editingRol ? 'Guardar Cambios' : 'Registrar Nuevo Rol'}</>
            )}
          </button>
        </div>
      </DialogContent>
