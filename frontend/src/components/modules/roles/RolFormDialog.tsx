import { Pencil, Shield, X, CheckCircle2, Loader2, Info } from 'lucide-react';
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <DialogDescription className="text-xs text-gray-400 mt-0.5">
                {editingRol ? 'Modifica los privilegios y descripción del rol' : 'Define los permisos y nombre para el nuevo rol administrativo'}
              </DialogDescription>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-6 space-y-8">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-gray-900 font-bold text-sm">Información General</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="bg-gray-50 border-gray-200 text-gray-800 rounded-xl h-11 focus:ring-[#c47b96]/20 focus:border-[#c47b96] transition-all"
                ...
                <div className="flex justify-between items-center px-1">
                  {(editingRol && (editingRol.id === '1' || editingRol.id === '2')) && (
                    <p className="text-[10px] text-amber-600 font-medium italic">Rol del sistema (no editable)</p>
                  )}
                  <span />
                  <p className="text-[10px] text-gray-400">{formData.nombre.length}/30</p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-5 bg-[#c47b96] rounded-full"></div>
              <h3 className="text-gray-900 font-bold text-sm">Matriz de Privilegios</h3>
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#fff0f5] border-b-2 border-[#fce8f0] hover:bg-[#fff0f5]">
                    <TableHead className="text-gray-700 font-bold text-[10px] uppercase tracking-wider py-4 pl-6">Módulo</TableHead>
                    <TableHead className="text-center text-gray-700 font-bold text-[10px] uppercase tracking-wider py-4">Ver</TableHead>
                    <TableHead className="text-center text-gray-700 font-bold text-[10px] uppercase tracking-wider py-4">Crear</TableHead>
                    <TableHead className="text-center text-gray-700 font-bold text-[10px] uppercase tracking-wider py-4">Editar</TableHead>
                    <TableHead className="text-center text-gray-700 font-bold text-[10px] uppercase tracking-wider py-4">Borrar</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {MODULOS.map((modulo) => (
                    <TableRow key={modulo.key} className="border-b border-gray-50 bg-white hover:bg-[#fff0f5]/30 group transition-colors">
                      <TableCell className="text-gray-800 font-bold text-xs pl-6 py-4">{modulo.label}</TableCell>
