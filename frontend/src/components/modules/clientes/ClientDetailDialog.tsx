...
export function ClientDetailDialog({
  ...
  const initials = `${cliente.nombres?.charAt(0) ?? ""}${cliente.apellidos?.charAt(0) ?? ""}`.toUpperCase();
  ...
  const hasLocation = cliente.departamento || cliente.ciudad || cliente.direccion;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-xl px-4">
                  <InfoRow
                    icon={CreditCard}
                    label={cliente.tipoDocumento || "Documento"}
                    value={cliente.numeroDocumento}
                  />
                  <InfoRow
                    icon={Calendar}
                    label="Fecha de Registro"
                    value={new Date(cliente.fechaRegistro).toLocaleDateString("es-CO", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-xl px-4">
                    {cliente.departamento && (
                      <InfoRow icon={Building2} label="Departamento" value={cliente.departamento} />
                    )}
                    {cliente.ciudad && (
                      <InfoRow icon={MapPin} label="Ciudad" value={cliente.ciudad} />
                    )}
                    {cliente.direccion && (
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-xl px-4">
                  <InfoRow icon={Mail} label="Correo electrónico" value={cliente.email} truncate />
                  <InfoRow icon={Phone} label="Teléfono" value={cliente.telefono || "No registrado"} />
                </div>
              </div>
            </div>
        ...
        <div className="px-6 pb-6 pt-2">
          <button
            onClick={() => onOpenChange(false)}
            className="w-full h-11 rounded-xl text-white font-semibold text-sm luxury-button-modal shadow-lg shadow-[#c47b96]/20 transition-all hover:opacity-90 cursor-pointer"
          >
            Cerrar
          </button>
        </div>
