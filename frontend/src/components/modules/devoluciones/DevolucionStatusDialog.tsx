import { Edit, CheckCircle2, Loader2, ShieldAlert } from 'lucide-react';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white border-0 w-[90vw] max-w-[800px] max-h-[90vh] flex flex-col rounded-2xl shadow-2xl p-0 overflow-hidden" style={{ '--input-background': '#ffffff' } as React.CSSProperties}>
        <DialogHeader className="px-4 md:px-8 pt-6 md:pt-8 pb-6 border-b border-gray-100 shrink-0 bg-white z-10">
          <div className="flex items-center gap-4">
            <div className="flex items-center justify-center text-white font-bold text-lg shrink-0" 
            ...
            <div>
              <DialogTitle className="text-xl font-semibold text-gray-900 leading-tight">Cambiar Estado</DialogTitle>
        ...
        <div className="px-4 md:px-8 py-6 space-y-6 overflow-y-auto flex-1 no-scrollbar">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-gray-700 font-medium text-sm">Nuevo Estado</Label>
            ...
            {nuevoEstado === "aprobada" && (
              <div className="space-y-3 animate-in fade-in slide-in-from-top-2 pt-2">
                <Label className="text-gray-700 font-medium text-sm">Estado Físico del Producto Devuelto <span className="text-rose-500">*</span></Label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div 
                    onClick={() => onEsDefectuosoChange(false)}
                    style={{ backgroundColor: !esDefectuoso ? '#fce7f3' : '#ffffff' }}
                    className={`p-4 rounded-2xl border-2 cursor-pointer transition-colors duration-300 flex flex-col items-center justify-center gap-2 ${!esDefectuoso ? 'border-[#c47b96] shadow-md' : 'border-gray-100 hover:border-gray-300'}`}
                  >
                     ...
                     <span className={`text-[13px] mt-1 font-medium ${!esDefectuoso ? 'text-[#c47b96]' : 'text-gray-500'}`}>Buen Estado</span>
                  ...
                  <div 
                    ...
                    className={`p-4 rounded-2xl border-2 cursor-pointer transition-colors duration-300 flex flex-col items-center justify-center gap-2 ${esDefectuoso ? 'border-[#c47b96] shadow-md' : 'border-gray-100 hover:border-gray-300'}`}
                  >
                     <div className={`p-2 rounded-full transition-colors duration-300 ${esDefectuoso ? 'bg-white shadow-sm' : 'bg-gray-50'}`}>
                       <ShieldAlert className={`w-6 h-6 transition-colors duration-300 ${esDefectuoso ? 'text-[#c47b96]' : 'text-gray-400'}`} />
                     </div>
                     <span className={`text-[13px] mt-1 font-medium transition-colors duration-300 ${esDefectuoso ? 'text-[#c47b96]' : 'text-gray-500'}`}>Producto Defectuoso</span>
            ...
            <div className="space-y-2">
              <Label className="text-gray-700 font-medium text-sm">Motivo de la Decisión <span className="text-rose-500">*</span></Label>
              <Textarea
                value={motivoDecision}
                onChange={(e) => onMotivoChange(e.target.value)}
                className="border-gray-200 text-gray-800 rounded-xl min-h-[100px] focus:ring-[#c47b96]/20 focus:border-[#c47b96] py-3 text-sm font-normal placeholder:text-gray-400 placeholder:opacity-50"
                style={{ backgroundColor: '#ffffff' }}
                placeholder="Explica detalladamente por qué se aprobó o rechazó esta solicitud de devolución..."
                disabled={isSaving}
              />
              <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest text-right">Mínimo: 3 caracteres</p>
            </div>
          </div>
          </div>

        <div className="flex gap-3 px-4 md:px-8 pb-6 pt-5 border-t border-gray-100 bg-white shrink-0 z-10">
          <button
