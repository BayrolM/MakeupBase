...
export function AsyncProductSelect({
  ...
  React.useEffect(() => {
    if (
      !inputValue ||
      (selectedProducto && inputValue === selectedProducto.nombre)
    ) {
  ...
  const searchProducts = async (searchQuery: string) => {
    ...
    try {
      ...
      setOptions(
        mapped.filter(
          (p) => p.estado === "activo" && (onlyWithStock ? p.stock > 0 : true),
        ),
      );
    } catch (e) {
      console.error(e);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    switch (e.key) {
      ...
      case "Enter":
        e.preventDefault();
        if (open && selectedIndex >= 0 && options[selectedIndex])
          ...
          >
            <div className="max-h-[250px] overflow-y-auto p-1 scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
              ...
              {!loading && options.length === 0 && (
                <div className="px-4 py-3 text-sm text-center text-gray-400 italic">
                  No se encontraron productos.
                </div>
              )}
              {!loading &&
                options.map((prod, index) => {
