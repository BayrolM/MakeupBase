  ...
  try {
    ...
    const resultado = await sql.begin(async (sql) => {
      ...
      for (const prod of productos) {
        ...
        if (prod.cantidad <= 0) throw new Error("La cantidad a devolver debe ser mayor a 0.");
      }

      // 4. Calcular total devuelto
      let totalDevuelto = 0;
      for (const prod of productos) totalDevuelto += prod.cantidad * prod.precio_unitario;
      ...
      for (const prod of productos) {
        await sql`
          ...
          VALUES (${devolucion.id_devolucion}, ${prod.id_producto}, ${prod.cantidad}, ${prod.precio_unitario}, ${prod.cantidad * prod.precio_unitario})
        `;
      }

      // 7. Si estado es "aprobada", manejar stock o pérdida
      if (estadoFinal === "aprobada") {
        if (es_defectuoso) {
          await registrarPerdidaPorDevolucion(sql, devolucion.id_devolucion, id_usuario_empleado, productos);
        } else {
          for (const prod of productos) {
            await sql`UPDATE productos SET stock_actual = stock_actual + ${prod.cantidad} WHERE id_producto = ${prod.id_producto}`;
          }
        }
      }
 ...
 * Si se aprueba, suma stock o registra pérdida si es defectuoso.
 */
export const cambiarEstado = async (req, res) => {
  try {
    ...
    await sql.begin(async (sql) => {
      const [devolucion] = await sql`SELECT * FROM devoluciones WHERE id_devolucion = ${id}`;
      ...
      if (estado === "aprobada") {
        const detalles = await sql`SELECT id_producto, cantidad FROM detalle_devoluciones WHERE id_devolucion = ${id}`;
        if (es_defectuoso) {
          await registrarPerdidaPorDevolucion(sql, id, id_usuario_empleado, detalles);
        } else {
          for (const det of detalles) {
            await sql`UPDATE productos SET stock_actual = stock_actual + ${det.cantidad} WHERE id_producto = ${det.id_producto}`;
          }
        }
      }
    });
...
export const anular = async (req, res) => {
  try {
    ...
    await sql.begin(async (sql) => {
      const [devolucion] = await sql`SELECT * FROM devoluciones WHERE id_devolucion = ${id}`;
      if (!devolucion) throw new Error("La devolución no existe.");
      if (devolucion.estado === "anulada") throw new Error("La devolución ya se encuentra anulada.");
      ...
      if (devolucion.estado === "aprobada") {
        if (!devolucion.es_defectuoso) {
          ...
          const detallesPerdida = await sql`SELECT DISTINCT id_perdida FROM detalle_perdidas WHERE id_devolucion = ${id}`;
          if (detallesPerdida.length > 0) {
            const idsPerdida = detallesPerdida.map(p => p.id_perdida);
            await sql`DELETE FROM detalle_perdidas WHERE id_devolucion = ${id}`;
            await sql`DELETE FROM perdidas WHERE id_perdida IN (${idsPerdida})`;
          }
        }
      }
...
async function registrarPerdidaPorDevolucion(sql, id_devolucion, id_usuario, productos) {
  ...
  for (const prod of productos) {
    const [infoProd] = await sql`SELECT costo_promedio FROM productos WHERE id_producto = ${prod.id_producto}`;
    const costo = Number(infoProd?.costo_promedio || 0);
    const subtotal = costo * prod.cantidad;
    totalPerdida += subtotal;
    detallesConCosto.push({ id_producto: prod.id_producto, cantidad: prod.cantidad, costo_unitario: costo, subtotal });
  }

  const observacion = `Perdida generada automaticamente por devolucion defectuosa #${id_devolucion}.`;
  const [perdida] = await sql`
    INSERT INTO perdidas (id_usuario_empleado, motivo, observacion, total_perdida)
    VALUES (${id_usuario}, 'otro', ${observacion}, ${totalPerdida})
  ...
  for (const det of detallesConCosto) {
    await sql`
      ...
      VALUES (${perdida.id_perdida}, ${det.id_producto}, ${det.cantidad}, ${det.costo_unitario}, ${det.subtotal}, ${id_devolucion})
    `;
  }
}
