...
export const obtener = async (req, res) => {
  try {
    const { id } = req.params;
    const result =
      await sql`SELECT * FROM proveedores WHERE id_proveedor = ${id}`;
    if (result.length === 0) {
      return res
        .status(404)
...
export const crear = async (req, res) => {
  try {
    ...
    if (!emailRegex.test(email)) {
      return res
        .status(400)
        .json({ ok: false, message: "Formato de correo inválido." });
    }
...
export const eliminar = async (req, res) => {
  try {
    ...
    try {
      const result =
        await sql`DELETE FROM proveedores WHERE id_proveedor = ${id} RETURNING *`;
      if (result.length === 0) {
        return res
          .status(404)
          .json({ ok: false, message: "Proveedor no encontrado." });
      ...
      if (dbError.code === "23503") {
        // Foreign key constraint violation in PostgreSQL
        return res
          .status(400)
          .json({
            ok: false,
            message:
              "Error al eliminar: El proveedor tiene registros asociados.",
          });
      }
      throw dbError;
    }
    ...
    return res
      .status(500)
      .json({ ok: false, message: "Error al desactivar el proveedor." });
  }
};
